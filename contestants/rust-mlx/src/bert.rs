...
impl BertEmbedder {
    ...
    pub fn embed_sentences(&self, sentences: Vec<&str>) -> Result<Array> {
    ...
    pub fn embed(
        ...
        let total_samples = input_ids.dim(0) as usize;
        let num_batches = (total_samples + batch_size - 1) / batch_size;
        let mut all_batch_embeddings: Vec<Array> = Vec::new();
        all_batch_embeddings.reserve_exact(num_batches);

        for batch_idx in 0..num_batches {
            let batch_start = batch_idx * batch_size;
            let batch_end = usize::min(batch_start + batch_size, total_samples);
            ...
            let batch_input_ids = input_ids.index(batch_start as i32..batch_end as i32);
            let batch_attention_masks = attention_mask.index(batch_start as i32..batch_end as i32);

            let result = self
                .model
                .forward(&batch_input_ids, &batch_attention_masks)?;
            result.eval()?;

            all_batch_embeddings.push(result);
        }
        let answer = mlx_rs::ops::concatenate_axis(&all_batch_embeddings, 0)?;
        answer.eval()?;
